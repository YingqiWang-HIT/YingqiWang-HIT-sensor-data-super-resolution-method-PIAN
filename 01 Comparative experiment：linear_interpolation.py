...
# 线性插值函数
def linear_interpolation(data, upscale_factor):
    """
    使用线性插值提升数据分辨率
    :param data: 原始数据
    ...
    interpolated_data = np.interp(x_high_res, x_original, data)  # 线性插值
...
def plot_results(original_data, high_res_data, interpolated_data, upscale_factor, output_dir, SRP):
    ...
    plt.show()
    output_excel_path = os.path.join(output_dir, f'Linear_Interpolation_Reslut_{SRP}.xlsx')
    # 将预测和标签保存到Excel
    df = pd.DataFrame({
        'Real': high_res_data,
...
if __name__ == "__main__":
    ...
    # 对训练数据进行线性插值提升分辨率
    interpolated_data = linear_interpolation(data_low_real, upscale_factor)
