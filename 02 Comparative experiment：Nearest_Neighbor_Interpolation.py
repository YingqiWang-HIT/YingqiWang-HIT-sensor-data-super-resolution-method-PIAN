...
# 线性插值函数
def nearest_neighbor_interpolation(data, upscale_factor):
    """
        使用最近邻插值提升数据分辨率
        :param data: 原始数据
    ...
    nearest_indices = np.round(x_high_res).astype(int)  # 将新索引四舍五入并转换为整数
    interpolated_data = data[nearest_indices]  # 通过最近邻插值生成高分辨率数据
...
def plot_results(original_data, high_res_data, interpolated_data, upscale_factor, output_dir, SRP):
    ...
    output_excel_path = os.path.join(output_dir, f'Nearest_Neighbor_Interpolation_Reslut_{SRP}.xlsx')
 # 将预测和标签保存到Excel
    df = pd.DataFrame({
        'Real': high_res_data,
...
if __name__ == "__main__":
    # 参数设置
    low_file_path = './data_set/FY-3A 飞轮E转速 降采样（×2, 缺失5%的数据）.xlsx'   # Excel文件路径
    high_file_path = './data_set/FY-3A 飞轮E转速.xlsx'   # Excel文件路径
    upscale_factor = 2  # 分辨率提升倍数
    ...
    # 对训练数据进行线性插值提升分辨率
    interpolated_data = nearest_neighbor_interpolation(data_low_real, upscale_factor)
